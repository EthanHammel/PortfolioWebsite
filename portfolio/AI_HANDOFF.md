SESSION_HANDOFF v6 — ETHAN HAMMEL PORTFOLIO
...
===SESSION_6_CHANGES===
ArtGallery.astro REWRITTEN — slideshow mode:
- One image in DOM at a time (was: all images rendered simultaneously)
- GIF fix: on navigate, src set to 1px transparent pixel FIRST (stops GIF decoding/memory), then 40ms setTimeout before loading next — lets browser GC old GIF
- Navigation: prev/next arrow buttons, dot indicators (≤20 items) or progress bar (>20 items), keyboard ← → arrows, touch swipe (dx>40px threshold)
- Loading state: img fades out (opacity 0, class is-loading), 3-dot pulse loader shown, fades back in on onload
- Metadata overlay: filename + ext badge, hover on desktop, tap-toggle on mobile (stage.classList.toggle('meta-open'))
- Counter: "N / total" DM Mono bottom-left
- Keyboard hint: "← → keys" DM Mono bottom-right, hidden on mobile
- Dots: .is-active = moss green, aria-selected for a11y, click goTo(i)
- Progress bar: shown instead of dots when >20 images, width% animates with CSS transition
- items serialized at build time via define:vars={{ itemsJson, count, useDots }}
art.css REWRITTEN:
- .slideshow wrapper with border + shadow
- .slideshow__stage: flex center, min-height 320px, max-height 72vh for img
- .slideshow__arrow: absolute positioned, opacity 0 → 1 on stage hover (always visible mobile)
- .slideshow__bar: flex space-between, border-top
- .slideshow__dot: 6px circle, moss when active
- .slideshow__progress + fill: 2px bar, moss fill, CSS transition on width
- @keyframes loader-pulse: 3 staggered dots

===SESSION_7_CHANGES===
CURSOR BIRD — full replacement of canvas-based system:
- OLD: canvas element + procedural drawing. GONE.
- NEW: DOM div + spritesheet background-position cycling. bird.js.
- PaperAirplane.astro: removed canvas, just loads /scripts/bird.js + global cursor:none CSS
- PNGs processed: birdsor_sh.png (210x60, 5 frames @ 42x60) → bird-sheet.png (630x180, 3x nearest-neighbor upscale)
- birdsor.png (42x60) → bird-sit.png (126x180, 3x upscale)
- FRAME_W=126 FRAME_H=180 NUM_FRAMES=5 FPS=10
- LAG_NORMAL=0.10 LAG_SNAP=0.28 (tighter on links/buttons)
- SIN_AMP=18px SIN_FREQ=0.045 rad/px — vertical sin wave while moving
- STILL_PX=2.5 threshold for moving vs sitting
- facingLeft: CSS scaleX(-1) flip only, NO rotation ever
- isOverLink: checks e.target.closest('a,button,[role=tab],input,label')
- Sitting: bird-sit.png shown, sin phase rounded to 2π for smooth restart
- Moving: cycles frames via backgroundPositionX = -(idx*FRAME_W)px
- cursor:none applied globally @media(min-width:640px)
- No canvas, no canvas in BaseLayout
